...
server <- function(input, output, session) {
  owmr_settings("af2f6b280ae76a3d215f081e0c014235")
    ...
    leaflet() %>%
      ...
      addMarkers(lat =weather$coord$lat, 
                 ...
                 popup = paste(
                   ...
                   "<b>Temperature: </b>",paste0(format(weather$main$temp, digits = 2), " C","<br>",
  ...
  output$tempBox <- renderInfoBox({
    weather <- data_current()
    infoBox(
      "Temperature", paste0(format(weather$main$temp, digits = 2), " °C"), icon = icon("temperature-low", lib = "font-awesome"),
      color = "yellow"
    )
  })
  ...
  output$hcontainer <- renderHighchart({
    ...
    température  <- as_tibble(data_forecast()$list)$main.temp
    
    températureMin  <- as_tibble(data_forecast()$list)$main.temp_min
    températureMax  <- as_tibble(data_forecast()$list)$main.temp_max
    ...
    highchart() %>%
      ...
      hc_add_series(name = "Température minimal", data=températureMin) |>
      hc_add_series(name = "Température maximal", data =températureMax) |>
